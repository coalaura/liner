...
function readSize(bytes, offset, synchsafe = false) {
	if (offset + 4 > bytes.length) {
		throw new Error("Truncated ID3 size.");
	}
	...
	for (let i = 0; i < 4; i++) {
		...
		if (synchsafe && byte > 127) {
			throw new Error("Invalid ID3 size.");
		}
...
function parseFrames(body, version, flags) {
	...
	while (offset < body.length) {
		...
		if (offset + 10 > body.length) {
			throw new Error("Truncated ID3 frame.");
		}

		const id = String.fromCharCode(...body.subarray(offset, offset + 4));

		if (!/^[A-Z0-9]{4}$/.test(id)) {
			throw new Error("Invalid ID3 frame identifier.");
		}
...
function mp3Frame(bytes, offset) {
	...
	const versionBits = (second >> 3) & 3,
		layerBits = (second >> 1) & 3,
		bitrateIndex = third >> 4,
		sampleRateIndex = (third >> 2) & 3;

	// MPEG version 01 is reserved. Layer bits 01 means Layer III.
	if (versionBits === 1 || layerBits !== 1 || bitrateIndex === 15 || sampleRateIndex === 3) {
		return null;
	}

	const version = versionBits === 3 ? 1 : versionBits === 2 ? 2 : 2.5,
		sampleRateDivisor = version === 1 ? 1 : version === 2 ? 2 : 4,
		sampleRate = [44100, 48000, 32000][sampleRateIndex] / sampleRateDivisor;

	// A bitrate index of zero is valid for free-format MP3 streams, but their frame length cannot be derived from a single header.
	if (bitrateIndex === 0) {
		return {
			version: version,
	...
	const bitrate = bitrates[bitrateIndex],
		padding = (third >> 1) & 1,
		coefficient = version === 1 ? 144000 : 72000,
		length = Math.floor(coefficient * bitrate / sampleRate) + padding;

	if (length < 24 || length > 8192) {
		return null;
	}
	...
	for (let offset = 0; offset + 4 <= bytes.length; offset++) {
		...
		if (frame.length !== null) {
			const nextOffset = offset + frame.length,
				next = mp3Frame(bytes, nextOffset);
		...
		for (let i = recentFrames.length - 1; i >= 0; i--) {
			...
			if (offset - previous.offset > maxFrameGap) {
				break;
			}
		...
		while (recentFrames.length && offset - recentFrames[0].offset > maxFrameGap) {
			recentFrames.shift();
		}
	}
...
async function inspectFile(file) {
	...
	if (file.size < 128) {
		throw new Error("This file is too small to be a valid MP3.");
	}

	const head = new Uint8Array(await file.slice(0, 10).arrayBuffer()),
		tail = new Uint8Array(await file.slice(-128).arrayBuffer());
	...
	if (head[0] === 0x49 && head[1] === 0x44 && head[2] === 0x33) {
		...
		if (version !== 3 && version !== 4) {
			throw new Error("This file uses ID3v2.2 or an unsupported ID3 version. Convert its tags to ID3v2.3 or ID3v2.4 first.",);
		}
		...
		if (start > file.size) {
			throw new Error("The ID3 header exceeds the file size.");
		}
	...
	const hasV1 = tail[0] === 0x54 && tail[1] === 0x41 && tail[2] === 0x47,
		end = file.size - (hasV1 ? 128 : 0);

	if (end <= start) {
		throw new Error("This file contains tags but no MP3 audio.");
	}
	...
	if (isMP4) {
		throw new Error("This file contains MP4/M4A audio, not MP3 audio. Renaming an MP4 file to .mp3 does not convert it.");
	}

	if (!hasMP3Audio(probe)) {
		throw new Error("Could not verify an MPEG Layer III stream near the start of the audio.");
	}
...
function decodeEntry(id, original, version) {
	...
	if (!definition || !definition.versions.includes(version)) {
		return rawEntry;
	}
	...
	const body = joinBytes(original),
		buffer = joinBytes([tagHeader(version, body.length), body]).buffer,
		reader = new MP3Tag(buffer);
	...
	const value = reader.tags.v2?.[id],
		expectsArray = definition.kind === "records" || definition.kind === "art";
...
function importLegacyTags(entries, tail, version) {
	...
	if (reader.error) {
		throw new Error(`Could not read ID3v1: ${reader.error}`);
	}

	const decoder = new TextDecoder("windows-1252");

	const text = (start, end) => decoder.decode(tail.subarray(start, end)).replace(/\0.*$/s, "").trimEnd();
...
function labeled(label, control) {
	const wrapper = element("div"),
		caption = element("span", "control-label", label);
...
function validTimestamp(value) {
	...
	const [, yearText, monthText, dayText, hourText, minuteText, secondText] = match,
		...
		month = monthText === undefined ? 1 : Number(monthText),
		day = dayText === undefined ? 1 : Number(dayText);
...
function makeControl(label, kind, value = "") {
	if (kind === "genre") {
		const root = element("div", "control-stack"),
			choices = [["", "Not set"], ...GENRE_CHOICES.map((genre) => [genre, genre])],
			code = /^\(?(\d+)\)?$/.exec(String(value));
		...
		const listed = choices.some(([key]) => key === value),
			select = selectControl(label, choices, listed ? value : "__custom__"),
			custom = basicControl("Custom genre", "text", listed ? "" : value);
	...
	if (kind === "rating") {
		const root = element("div"),
			range = element("input"),
			output = element("output", "rating-value");
	...
	if (kind === "timestamp") {
		const root = element("div", "split-control date-control"),
			year = basicControl(`${label}: year`, "year", String(value).slice(0, 4)),
			remainder = basicControl(`${label}: date and time`, "text", String(value).slice(4));
		...
		return {
			...
			read() {
				...
				checkInput(
					remainder.node,
					label,
					result && !validTimestamp(result) ? "Use YYYY, YYYY-MM, YYYY-MM-DD or YYYY-MM-DDTHH:mm:ss." : ""
				);
	...
	if (kind === "position") {
		if (String(value).includes(MULTI_VALUE_SEPARATOR)) {
			...
			control.read = () => {
				const result = read(),
					valid = result.split(MULTI_VALUE_SEPARATOR).every((part) => /^\d+(\/\d+)?$/.test(part));
		...
		const root = element("div", "split-control"),
			[number = "", total = ""] = String(value).split("/"),
			position = basicControl(`${label}: number`, "integer", number),
			count = basicControl(`${label}: total`, "integer", total);
		...
		return {
			...
			read() {
				...
				checkInput(count.node, label, total && Number(total) < Number(number) ? "The total cannot be smaller than the number." : "");
...
function recordEditor(entry) {
	const root = element("div"),
		list = element("div", "record-list"),
		records = [],
		schema = RECORD_FIELDS[entry.id];

	function addRecord(value = {}, dirty = true) {
		const card = element("div", "record"),
			heading = element("div", "record-heading"),
			fields = element("div", "record-fields"),
			controls = new Map(),
			record = {
				card: card,
				controls: controls
	...
	root.append(list, button("＋ Add entry", "button button-quiet", () => addRecord()));
...
function detectImageMime(bytes, offset = 0) {
	...
	if (bytes[offset] === 0x89 && bytes[offset + 1] === 0x50 && bytes[offset + 2] === 0x4e && bytes[offset + 3] === 0x47 && bytes[offset + 4] === 0x0d && bytes[offset + 5] === 0x0a && bytes[offset + 6] === 0x1a && bytes[offset + 7] === 0x0a) {
		return "image/png";
	}
...
async function jpegArtwork(file) {
	...
	try {
		if (!bitmap.width || !bitmap.height || bitmap.width * bitmap.height > MAX_IMAGE_PIXELS || bitmap.width > 8192 || bitmap.height > 8192) {
		...
		if (!context) {
			throw new Error("A canvas could not be created.");
		}
...
function renderArtwork(entry) {
	...
	function addPicture(value, dirty = false) {
		const picture = { ...value, url: null },
			card = element("div", "picture-card"),
			preview = element("div", "picture-preview"),
			controls = element("div", "picture-controls"),
			actions = element("div", "raw-actions"),
			information = element("p", "picture-info");
		...
		function updatePreview() {
			...
			try {
				...
				preview.append(element("span", "", "Preview unavailable · original image retained"));
			...
			picture.url = URL.createObjectURL(new Blob([imageData.bytes], {
				type: imageData.mime
			}));
			...
			image.addEventListener("error", () => {
				...
				preview.replaceChildren(element("span", "", "The embedded image could not be decoded."));
			}, { once: true });
	...
	entry.read = () => {
		...
		for (let index = 0; index < values.length; index++) {
			...
			if (usedDescriptions.has(description)) {
				checkInput(pictures[index].descriptionInput, "Artwork description", "Use a unique description for each artwork image.");
			}
		...
		for (let index = 0; index < values.length; index++) {
			if (values[index].description.trim()) {
				continue;
			}

			const base = PICTURE_TYPES[values[index].type] || `Artwork ${index + 1}`;
...
function rawEditor(entry) {
		...
		input.addEventListener("change", () => {
			...
			perform(`Replacing ${entry.id}…`, async () => {
				...
				const payload = new Uint8Array(await file.arrayBuffer()),
					header = record.bytes.slice(0, 10);
	...
	return {
		node: root,
		read: () => records.map((record) => record.bytes)
	};
}
...
function refreshPicker() {
	...
	const available = TAGS.filter((tag) => (
		tag.id !== "APIC" && tag.versions.includes(state.version) && !state.entries.has(tag.id)
	));
...
function renderEntry(entry) {
	...
	const wide = entry.raw || entry.definition.kind === "records",
		card = element("section", `tag-card${wide ? " wide" : ""}`),
		heading = element("div", "tag-heading"),
		title = entry.raw ? entry.id : entry.definition.label;
...
async function openFile(file, askToDiscard = true) {
	...
	if (askToDiscard && state?.changed && !window.confirm("Discard your current changes and open another file?")) {
		return;
	}
	...
	await perform("Reading tags…", async () => {
		...
		state = { ...source, entries: entries, changed: false };
		...
		notify(source.hasAPE ? "Loaded. APEv2 metadata is retained unchanged; this editor edits ID3 tags." : "Loaded. Drop another MP3 anywhere to switch files.", "success",);
	});
}
...
function encodeEditedEntry(entry) {
	...
	const encoded = new Uint8Array(writer.buffer),
		length = readSize(encoded, 6, true);
...
ui.artInput.addEventListener("change", () => {
	...
	if (files.length) {
		acceptOneFile(files, addArtwork);
	}
...
ui.reset.addEventListener("click", () => {
	...
	if (state.changed && !window.confirm("Reset all edits to the original file?")) {
		return;
	}
